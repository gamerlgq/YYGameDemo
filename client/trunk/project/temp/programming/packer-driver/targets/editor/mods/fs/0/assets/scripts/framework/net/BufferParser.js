  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", BufferParser = class BufferParser {
        ...
        constructor() {
          ...
          if (!BufferParser.pool) {
            BufferParser.pool = new Pool(() => {
              let arrayBuffer = new ArrayBuffer(BUFF_SIZE); //发送缓冲区
        ...
        generateMsgArrayBuffer(msgId, ...sendParams) {
          let ab = BufferParser.pool.alloc();
          this._dataView = new DataView(ab);
          let msgParams = (_crd && Proto === void 0 ? (_reportPossibleCrUseOfProto({
            error: Error()
        ...
        _writeData(msgParams, ...sendParams) {
          for (let index = 0; index < msgParams.length; index++) {
            ...
            if (typeStr == "uint8" || typeStr == "int8") {
              ...
              let byteArray = (_crd && encodeUtf8 === void 0 ? (_reportPossibleCrUseOfencodeUtf({
                error: Error()
        ...
        _parseData(msgParams, arrayBuffer, dataRet) {
          ...
          for (let index = 0; index < msgParams.length; index++) {
            ...
            if (typeStr == "uint8" || typeStr == "int8") {
              ...
              let str = (_crd && decodeUtf8 === void 0 ? (_reportPossibleCrUseOfdecodeUtf({
                error: Error()
