      ...
      _export("MulitMoveingBgs", MulitMoveingBgs = (_dec4 = ccclass('MulitMoveingBgs'), _dec5 = property({
        ...
        onMainBgChangePos(insreasedPos) {
          this.subBg.forEach(element => {
            var subMovePos = insreasedPos.clone().multiplyScalar(element.moveScale);
        ...
        move(byPos, isDInertia) {
          // 惯性移动
          if (isDInertia) {
            var dInertiaMoveComp = this.mainBg.getComponent(_crd && DInertiaMove === void 0 ? (_reportPossibleCrUseOfDInertiaMove({
              error: Error()
            ...
            // 非惯性移动
            //判断结束
            var newPos = this.mainBg.position.clone().add(v3(byPos.x, byPos.y));
            ...
            var insreasedPos = newPos.subtract(this.mainBg.position);
            var dis = insreasedPos.length();
