    ...
    execute: function () {
      ...
      _export("TableLayer", TableLayer = class TableLayer extends Node {
        ...
        appendSubContent(layer) {
          ...
          layer.on("exit", () => {
            // 如果发现顶层没有pop掉就pop一下
            var topLayer = this._subContentList[this._subContentList.length - 1];
        ...
        popSubContent() {
          var layer = this._subContentList.pop();
        ...
        isEmpty() {
          var childCount = this._uiLayer.children.length + this._contentLayer.children.length + this._bgLayer.children.length + this._subContent.children.length;
          return 0 >= childCount;
        }

        setContentLayerVisible(flag) {
          var contentLayer = this.getContentLayer();
