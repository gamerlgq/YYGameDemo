    ...
    execute: function () {
      ...
      _export("DataRegisterMgr", DataRegisterMgr = class DataRegisterMgr extends (_crd && Singleton === void 0 ? (_reportPossibleCrUseOfSingleton({
        ...
        loadAllData(doneFunc) {
          ...
          var startTS = new Date().getMilliseconds();
          var length = Object.keys(this.DataType).length;

          for (var key in this.DataType) {
            if (Object.prototype.hasOwnProperty.call(this.DataType, key)) {
              (function () {
                var value = _this.DataType[key];
                var dataHandlerName = value[0];
                var path = value[1];
                var pair = value[2];
                var parser = value[3];
                ...
                (_crd && dataMgr === void 0 ? (_reportPossibleCrUseOfdataMgr({
                  ...
                  if (length == 0) {
                    if (doneFunc) {
                      var endTS = new Date().getMilliseconds();
