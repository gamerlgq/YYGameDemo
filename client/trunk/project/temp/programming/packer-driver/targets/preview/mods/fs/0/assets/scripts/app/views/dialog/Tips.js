  ...
  return {
    ...
    execute: function () {
      ...
      _export("Tips", Tips = (_dec = ccclass('Tips'), _dec(_class = (_temp = class Tips extends Component {
        ...
        start() {
          // [3]
          var contentNode = find("content", this.node);
        ...
        set text(value) {
          if (!this._bg) {
            ...
            this._labelNode.on(Node.EventType.SIZE_CHANGED, () => {
              var width = this._labelNode.getComponent(UITransform).width;

              var bgUITrans = this._bg.getComponent(UITransform);
          ...
          var labelComp = this._labelNode.getComponent(Label);
