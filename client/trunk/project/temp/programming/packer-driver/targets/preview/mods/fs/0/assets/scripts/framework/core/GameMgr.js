  ...
  return {
    ...
    execute: function () {
      ...
      _export("GameMgr", GameMgr = class GameMgr extends (_crd && Singleton === void 0 ? (_reportPossibleCrUseOfSingleton({
        ...
        _fastTick(dt) {
          // handler inner msg
          var innerlenght = this._innerMessageQueue.length;

          while (innerlenght > 0) {
            var msgEvent = this._innerMessageQueue.shift();
          ...
          var serverlenght = this._serverMessageQueue.length;

          while (serverlenght > 0) {
            var _msgEvent = this._serverMessageQueue.shift();

            this._dispatchMsgEvent(_msgEvent);
